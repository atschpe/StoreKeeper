...
import com.example.android.storekeeper.data.StoreContract;
...
public class CatalogActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>  {
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        String[] projection = {
                ...
                StoreContract.ItemEntry.ITM_IMAGE,
                StoreContract.ItemEntry.ITM_NAME,
                StoreContract.ItemEntry.ITM_PRICE,
                StoreContract.ItemEntry.ITM_QUANTITY,
                StoreContract.ItemEntry.ITM_DESCRIPTION};
        return new CursorLoader(this, StoreContract.ItemEntry.CONTENT_URI, projection, null, null, null);
    }
    ...
    private void addDummyItem() {
        ...
        //dummy image.
        int imageResource = R.drawable.ic_dummy;
        String imageData = String.valueOf(StoreContract.ItemEntry.DUMMY_IMG);

        //Gather values from dummy
        ContentValues values = new ContentValues();
        values.put(StoreContract.ItemEntry.ITM_NAME, "Dummy");
        values.put(StoreContract.ItemEntry.ITM_IMAGE, imageData);
        values.put(StoreContract.ItemEntry.ITM_PRICE, 10.00);
        values.put(StoreContract.ItemEntry.ITM_QUANTITY, 5);
        values.put(StoreContract.ItemEntry.ITM_SUP_MAIL, 0);
        values.put(StoreContract.ItemEntry.ITM_DESCRIPTION, getString(R.string.item_description));
        values.put(StoreContract.ItemEntry.ITM_ORDER_NO, "000000");

        Uri addDummyItem = this.getContentResolver().insert(StoreContract.ItemEntry.CONTENT_URI, values);
        Log.v(LOG_TAG, "Dummy added to item table of the database.");
    }

    private void deleteAllItems() {
        int rowsDeleted = this.getContentResolver().delete(StoreContract.ItemEntry.CONTENT_URI, null, null);
